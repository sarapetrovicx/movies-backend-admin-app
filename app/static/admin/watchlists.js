...
////////////////// LOAD
    ...
    fetch('http://127.0.0.1:8080/api/watchlists', {
        method:"GET",
        headers: {
...
////////////////// DELETE
        ...
        for(i=0; i<btns_obrisi.length; i++){
            btns_obrisi[i].addEventListener("click", function(){
                fetch('http://127.0.0.1:8080/api/watchlists/'+this.parentNode.parentNode.dataset.taskid,
                { method:"DELETE", headers:{'Authorization': `Bearer ${token}`}})
                .then( response=>response.json())
                .then( data => {fetch('http://127.0.0.1:8080/api/watchlists', { method:"GET", headers:{'Authorization': `Bearer ${token}`}})
                .then(response => response.json())
        ...
        for(i=0; i < btns_zavrsis.length; i++){
            btns_zavrsis[i].addEventListener("click", function(){
                ...
                document.getElementById("save").addEventListener("click", function(){
                    ...
                    fetch('http://127.0.0.1:8080/api/watchlists/edit/'+ id,{
                        method:"PUT",
                    ...
                    .then( data => {
                    fetch('http://127.0.0.1:8080/api/watchlists', { method:"GET", headers:{'Authorization': `Bearer ${token}`}})
                    .then(response => response.json())
                    .then(data => updateTaskList(data) );
                    }).catch(err => alert(err));
                })
...
////////////////// FIND
    document.getElementById("btn_filter").addEventListener("click",function(){
        ...
        fetch('http://127.0.0.1:8080/api/watchlists/find/'+q, { method:"GET", headers:{'Authorization': `Bearer ${token}`}})
            .then( response=>response.json())
            .then( data => updateTaskList(data) );
    });
////////////////// LOGOUT
