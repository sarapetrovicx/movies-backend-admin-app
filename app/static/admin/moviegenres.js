...
////////////////// LOAD
    ...
    fetch('http://127.0.0.1:8080/api/moviegenres', {
        method:"GET",
        headers: {
...
////////////////// DELETE
        ...
        for(i=0; i<btns_obrisi.length; i++){
            btns_obrisi[i].addEventListener("click", function(){
                fetch('http://127.0.0.1:8080/api/moviegenres/'+this.parentNode.parentNode.dataset.taskid,
                { method:"DELETE", headers:{'Authorization': `Bearer ${token}`}})
                .then( response=>response.json())
                .then( data => {fetch('http://127.0.0.1:8080/api/moviegenres', { method:"GET", headers:{'Authorization': `Bearer ${token}`}})
                .then(response => response.json())
        ...
        for(i=0; i < btns_zavrsis.length; i++){
            btns_zavrsis[i].addEventListener("click", function(){
                ...
                document.getElementById("save").addEventListener("click", function(){
                    ...
                    fetch('http://127.0.0.1:8080/api/moviegenres/edit/'+ id,{
                        method:"PUT",
                    ...
                    .then( data => {
                    fetch('http://127.0.0.1:8080/api/moviegenres', { method:"GET", headers:{'Authorization': `Bearer ${token}`}})
                    .then(response => response.json())
                    .then(data => updateTaskList(data) );
                    }).catch(err => alert(err));
                })
...
////////////////// ADD
    this.document.getElementById("btn_dodaj").addEventListener("click", function(){
        let novi = {
            ...
            genreId: document.getElementById("novi_task_datum").value,
        };
        console.log(novi);
        nt = JSON.stringify(novi);
        fetch('http://127.0.0.1:8080/api/moviegenres',{
    ...
    document.getElementById("btn_filter").addEventListener("click",function(){
        ...
        fetch('http://127.0.0.1:8080/api/moviegenres/find/'+q, { method:"GET", headers:{'Authorization': `Bearer ${token}`}})
            .then( response=>response.json())
            .then( data => updateTaskList(data) );
    });
////////////////// LOGOUT
