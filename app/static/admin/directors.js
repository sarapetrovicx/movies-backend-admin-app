    ...
    function updateTaskList(directors){
        ...
        for(i in directors){
            ...
            `<tr data-taskID="`+directors[i].id+`">
                ...
                <td>`+directors[i].first_name+`</td>
                <td>`+directors[i].last_name+`</td>
                <td>
...
////////////////// DELETE
        ...
        for(i=0; i<btns_obrisi.length; i++){
            btns_obrisi[i].addEventListener("click", function(){
                fetch('http://127.0.0.1:8080/api/directors/'+this.parentNode.parentNode.dataset.taskid,
                { method:"DELETE", headers:{'Authorization': `Bearer ${token}`}})
                .then( response=>response.json())
                .then( data => {fetch('http://127.0.0.1:8080/api/directors', { method:"GET", headers:{'Authorization': `Bearer ${token}`}})
                .then(response => response.json())
            ...
            btns_edits[i].addEventListener("click", function(){
                ...
                document.getElementById("save").addEventListener("click", function(){
                    let novi = {
                        ...
                        last_name: document.getElementById("email").value,
                    };
                    nt = JSON.stringify(novi);
                    fetch('http://127.0.0.1:8080/api/directors/edit/'+ id,{
                        method:"PUT",
                    ...
                    .then( data => {
                    fetch('http://127.0.0.1:8080/api/directors', { method:"GET", headers:{'Authorization': `Bearer ${token}`}})
                    .then(response => response.json())
                    .then(data => updateTaskList(data) );
                    }).catch(err => alert(err));
                })
...
////////////////// ADD
    this.document.getElementById("btn_dodaj").addEventListener("click", function(){
        let novi = {
            first_name: document.getElementById("novi_task_naziv").value,
            last_name: document.getElementById("novi_task_datum").value,
        };
        console.log(novi);
        nt = JSON.stringify(novi);
        fetch('http://127.0.0.1:8080/api/directors',{
...
////////////////// FIND
    document.getElementById("btn_filter").addEventListener("click",function(){
        ...
        fetch('http://127.0.0.1:8080/api/directors/find/'+q, { method:"GET", headers:{'Authorization': `Bearer ${token}`}})
        .then( response=>response.json())
        .then( data => updateTaskList(data) );
    });
////////////////// LOGOUT
