...
////////////////// LOAD
    ...
    fetch('http://127.0.0.1:8080/api/genres', {
        method:"GET",
        headers: {
    ...
    function updateTaskList(genres){
        var tabela = document.getElementById("korisnici");
        tabela.innerHTML = "";
        for(i in genres){
            let redHTML = 
            `<tr data-taskID="`+genres[i].id+`">
                <td>`+genres[i].id+`</td>
                <td>`+genres[i].genre_name+`</td>
                <td>
...
////////////////// DELETE
    ...
    for(i=0; i<btns_obrisi.length; i++){
        btns_obrisi[i].addEventListener("click", function(){
            fetch('http://127.0.0.1:8080/api/genres/'+this.parentNode.parentNode.dataset.taskid,
            { method:"DELETE", headers:{'Authorization': `Bearer ${token}`}})
            .then( response=>response.json())
            .then( data => {fetch('http://127.0.0.1:8080/api/genres', { method:"GET", headers:{'Authorization': `Bearer ${token}`}})
            .then(response => response.json())
        ...
        for(i=0; i < btns_zavrsis.length; i++){
            btns_zavrsis[i].addEventListener("click", function(){
                ...
                document.getElementById("save").addEventListener("click", function(){
                    let novi = {
                        genre_name: document.getElementById("name").value,
                    };
                    nt = JSON.stringify(novi);
                    fetch('http://127.0.0.1:8080/api/genres/edit/'+ id,{
                        method:"PUT",
                    ...
                    .then( data => {
                    fetch('http://127.0.0.1:8080/api/genres', { method:"GET", headers:{'Authorization': `Bearer ${token}`}})
                    .then(response => response.json())
                    .then(data => updateTaskList(data) );
                    }).catch(err => alert(err));
                })
...
////////////////// ADD
    this.document.getElementById("btn_dodaj").addEventListener("click", function(){
        let novi = {
            genre_name: document.getElementById("novi_task_naziv").value,
        };
        console.log(novi);
        nt = JSON.stringify(novi);
        fetch('http://127.0.0.1:8080/api/genres',{
...
////////////////// FIND
    document.getElementById("btn_filter").addEventListener("click",function(){
        ...
        fetch('http://127.0.0.1:8080/api/genres/find/'+q, { method:"GET", headers:{'Authorization': `Bearer ${token}`}})
            .then( response=>response.json())
            .then( data => updateTaskList(data) );
    });
////////////////// LOGOUT
