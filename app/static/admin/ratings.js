...
////////////////// LOAD
    ...
    fetch('http://127.0.0.1:8080/api/ratings', {
        method:"GET",
        headers: {
...
////////////////// DELETE
        ...
        for(i=0; i<btns_obrisi.length; i++){
            btns_obrisi[i].addEventListener("click", function(){
                fetch('http://127.0.0.1:8080/api/ratings/'+this.parentNode.parentNode.dataset.taskid,
                { method:"DELETE", headers:{'Authorization': `Bearer ${token}`}})
                .then( response=>response.json())
                .then( data => {fetch('http://127.0.0.1:8080/api/ratings', { method:"GET", headers:{'Authorization': `Bearer ${token}`}})
                .then(response => response.json())
        ...
        for(i=0; i < btns_zavrsis.length; i++){
            btns_zavrsis[i].addEventListener("click", function(){
                ...
                document.getElementById("save").addEventListener("click", function(){
                    let novi = {
                        rate: document.getElementById("name").value,
                        userId: document.getElementById("genre").value,
                        movieId: document.getElementById("movie").value,
                    };
                    nt = JSON.stringify(novi);
                    fetch('http://127.0.0.1:8080/api/ratings/edit/'+ id,{
                        method:"PUT",
                    ...
                    .then( data => {
                    fetch('http://127.0.0.1:8080/api/ratings', { method:"GET", headers:{'Authorization': `Bearer ${token}`}})
                    .then(response => response.json())
                    .then(data => updateTaskList(data) );
                    }).catch(err => alert(err));
                    });
...
////////////////// FIND
    document.getElementById("btn_filter").addEventListener("click",function(){
        ...
        fetch('http://127.0.0.1:8080/api/ratings/find/'+q, { method:"GET", headers:{'Authorization': `Bearer ${token}`}})
            .then( response=>response.json())
            .then( data => updateTaskList(data) );
    });
////////////////// LOGOUT
